...
def main():
    ...
    # eva02
    model = timm.create_model('eva02_base_patch14_224.mim_in22k', pretrained=True, num_classes=num_classes)
    checkpoint = torch.load('eva02_best_ft.pth', weights_only=True)
    ...
    # Training Data
    fer_dataset_train = DataProcessor(csv_file=r"train_labels.csv", img_dir=r"train", transform=train_transform)
    train_loader = DataLoader(fer_dataset_train, batch_size=16, shuffle=True, num_workers=8, persistent_workers=True, pin_memory=True)

    # Validation Data
    val_dataset = DataProcessor(csv_file=r"val_labels.csv", img_dir=r"val", transform=val_transform)
    ...
    class_weights = compute_class_weights(r"train_labels.csv", 7)
    ...
    criterion = torch.nn.CrossEntropyLoss(weight=fer_weights, label_smoothing=0.1)
    ...
    checkpoint = torch.load('eva02_best.pth', weights_only=True)
    model.load_state_dict(checkpoint['model_state_dict'])
